...
dict_file = "drug_ner_dictionary.pkl.bz2"
with bz2.open(this_path.joinpath(dict_file), "rb") as f:
    d = pkl.load(f)
...
def find_drugs(tokens: list, is_fuzzy_match=False, is_ignore_case=None, is_include_structure=False,
    ...
    def inject_smiles(match_data, canonical_name):
        if "smiles" in drug_canonical_to_data.get(canonical_name, {}):
            match_data["smiles"] = drug_canonical_to_data[canonical_name]["smiles"]
